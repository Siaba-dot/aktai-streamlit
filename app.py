...
FMT_MONEY = "#,##0.00"   # 2 dešimtainiai – sumoms (be PVM/PVM/su PVM)
FMT_QTY   = "#,##0.00"   # 2 dešimtainiai – kiekiui
FMT_RATE  = "0.0000"     # 4 dešimtainiai – ĮKAINIUI (be PVM)
...
def dec2(v) -> float:
    """Apvalina iki 2 dešimtainių (kiekio ir sumų skaičiavimams)."""
    return float(Decimal(str(v).replace(",", ".")).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP))

def dec4(v) -> float:
    """Apvalina iki 4 dešimtainių (ĮKAINIUI be PVM)."""
    return float(Decimal(str(v).replace(",", ".")).quantize(Decimal("0.0001"), rounding=ROUND_HALF_UP))
...
def prepare_rows_for_address(
    ...
    # KIEKIS – 2 d.; ĮKAINIS – 4 d. (tiksliai)
    dfp["__qty"]  = dfp["__qty"].apply(dec2)
    dfp["__rate"] = dfp["__rate"].apply(dec4)  # <-- svarbiausia pataisa: 4 dešimtainiai
...
def write_address_block(
    ...
    for i, (service, qty, rate) in enumerate(rows, start=1):
        ...
        ws[f"D{r}"].number_format = FMT_RATE   # <-- ĮKAINIS – visada 4 dešimtainiai
...
def build_workbook_act_multi(
    ...
    for addr in addresses_selected:
        ...
        rows = prepare_rows_for_address(
            dfa, col_map["service"], col_map["qty"], col_map["rate"],
            selected_for_addr, group_same
        )
        if not rows:
...
if btn:
    try:
        wb = build_workbook_act_multi(
            df_raw, col_map, contract_no or "", manager,
            addresses_selected, service_selection_map, akto_data, group_same
        )
