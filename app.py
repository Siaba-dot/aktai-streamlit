...
# -*- coding: utf-8 -*-
"""
Aktų generatorius (Streamlit Cloud-ready):
...
- NĖRA ankstyvo st.stop prieš UI (kad neatsirastų „baltas“ ekranas).
"""
...
ROW_TABLE_START = 9
MAX_LINES = 40
SEP = ","  # jei tavo Excel naudoja ';' formulėms – pakeisk į ";"
...
def build_workbook_consolidated(
    ...
    r = 1
    ws[f"A{r}"] = f"Bendras aktas pagal sutartį: {contract_no}"; ws[f"A{r}"].font = Font(size=12, bold=True); r += 2
    ...
    if subtotal_refs:
        ws[f"D{r}"] = "Bendra suma (be PVM):"; ws[f"E{r}"] = f"=SUM({','.join(subtotal_refs)})"; ws[f"E{r}"].number_format = FMT_MONEY; r += 1
        ws[f"D{r}"] = f"PVM {float(PVM_DEFAULT)}%:"; ws[f"E{r}"] = f"=E{r-1}*{float(PVM_DEFAULT)/100}"; ws[f"E{r}"].number_format = FMT_MONEY; r += 1
        ws[f"D{r}"] = "Bendra suma su PVM:"; ws[f"E{r}"] = f"=E{r-2}+E{r-1}"; ws[f"E{r}"].number_format = FMT_MONEY
        set_borders(ws, f"D{r-2}:E{r}", thick=True)
...
# ===== STREAMLIT UI (jokio ankstyvo stop) =====
...
if btn1 and selected_key:
    try:
        ...
        st.download_button("Atsisiųsti aktą", xlsx, "aktas_dropdown_paieška.xlsx",
                           mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        cleaned = clean_before_sending_bytes(xlsx)
        st.download_button("Atsisiųsti švarų aktą", cleaned, "aktas_dropdown_paieška_clean.xlsx",
                           mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    except Exception as e:
        st.exception(e)
