...
- UI: sutarties pasirinkimas; (pasirenkami) FILTRAI pagal UŽSAKOVĄ ir VADYBININKĄ; pasirinkti KELIS ADRESUS (visi iš tos pačios sutarties ir to paties vadybininko, jei priskirtas);
  GLOBALUS PASLAUGŲ FILTRAS; po adresų pasirinkimo – PASLAUGŲ MULTISELECT KIEKVIENAM ADRESUI.
- Kiekvienam adresui: „Atliktų paslaugų data: 2026 m.“ -> „Objekto adresas …, Skyrius …“ -> lentelė su fiksuotais pločiais (A=6, B=48, C=10, D=12, E=14), wrap B.
- „Group by“ (pagal paslaugos pavadinimą ir įkainį) – sujungiamos eilutės, sumuojamas kiekis.
- Apačioje: BENDRA SUMA ir PARAŠŲ BLOKAS dviem stulpeliais (Užsakovas / Vykdytojas) A–E ribose, be datų.
"""
...
def create_named_styles(wb: Workbook) -> None:
    ...
    # NamedStyle instancijos negalima dauginti su tuo pačiu vardu; tikrinam pagal vardą
    if "Money" not in [s.name for s in wb.named_styles]:
        stl = NamedStyle(name="Money")
        stl.number_format = FMT_MONEY
        stl.alignment = Alignment(horizontal="right")
        wb.add_named_style(stl)
    if "Qty" not in [s.name for s in wb.named_styles]:
...
# ===== Paruošiame eilutes vienam adresui (grupavimas, filtrai pagal paslaugas) =====
...
def write_address_block(
    ...
    # Lentele header (fiksuotos kolonos; wrap B)
...
def build_workbook_act_multi(
    ...
    Toliau – blokai kiekvienam adresui (su filtrais per-adresą), tada bendra suma ir parašų blokas A–E ribose.
    """
    ...
    for addr in addresses_selected:
        ...
        # Pasirinktos paslaugos šiam adresui (jei yra)
        ...
        if not rows:
            ...
            # (alternatyva: raise ValueError, bet elegantiškiau tiesiog praleisti)
            continue
...
# ===== Adresų pasirinkimas (multiselect; po galutinių filtrų) =====
...
# ===== Per-adresą paslaugų pasirinkimas + skyrių rodymas =====
service_selection_map: Dict[str, List[str]] = {}
if addresses_selected:
    ...
    for addr in addresses_selected:
        ...
        # Paslaugos šiam adresui (po visų filtruojančių veiksmų)
