...
# app.py
# Streamlit Cloud Atliktų darbų aktų generatorius (Excel, be PDF)
...
# ------------------------------------------------------------
# PIRMIAUSIA Streamlit komanda: turi būti prieš bet kokią kitą st.* eilutę
# ------------------------------------------------------------
st.set_page_config(page_title="Aktų generatorius", page_icon="📄", layout="wide")

# Build žyma, kad matytum, jog startavo naujas kodas
st.caption("build: v2026-01-04-14:45 (app.py)")

# ------------------------------------------------------------
# Reikalaujami stulpeliai duomenų faile
# (šių pakanka akto generavimui; VVS/Galioja iki/Vadybininkas akte nenaudojami)
# ------------------------------------------------------------
REQUIRED_COLS = [
    "Skyrius",
    "Objekto adresas",
...
with st.sidebar:
    ...
    sujungti_i_viena_faila = st.checkbox("Sujungti į vieną .xlsx (atskiri lapai)", value=False)
    st.markdown("---")
...
def sanitize_filename(name: str) -> str:
    """Saugus failo pavadinimo paruošimas (be draudžiamų simbolių ir nereikalingų tarpų)."""
    name = str(name).strip()
...
def render_header(ws, wb, start_row, meta: dict):
    """Akto antraštėje paliekam: Užsakovas, Vykdytojas, Sutarties nr., Adresas, Skyrius, Akto data."""
...
def write_act_to_sheet(wb, sheet_name: str, meta: dict, items: pd.DataFrame, pvm_pct: float, show_pvm: bool):
    ...
    # Eilučių rašymas (patikimai: dirbam su žodynų įrašais)
...
# ------------------------------------------------------------
if uploaded:
    df = read_excel_to_df(uploaded.read())
