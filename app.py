...
- Užsakovas -> Vykdytojas -> (pavadinimas „Atliktų paslaugų aktas“, bold + didesnis, centruotas per A–E) -> tuščia eilutė ->
  Sutarties nr. -> tuščia eilutė.
- UI: sutarties pasirinkimas, (pasirenkamas) vadybininko filtras; keli adresai, visi iš tos pačios sutarties ir to paties vadybininko (jei priskirtas).
- Prieš generavimą: rodyti sutarties nr.; pasirinkus adresus – rodyti jų skyrius.
- Kiekvienam adresui: „Atliktų paslaugų data: 2026 m.“ -> „Objekto adresas …, Skyrius …“ -> lentelė su paslaugomis.
- „Group by“ (pagal paslaugos pavadinimą ir įkainį) – sujungiamos eilutės, sumuojamas kiekis.
- Fiksuoti stulpelių pločiai; B stulpelis wrap; A siauras („Eil. Nr.“).
- Apačioje: PARAŠŲ BLOKAS dviem stulpeliais (Užsakovas / Vykdytojas) A–E ribose, be datų.
"""
...
def set_table_column_widths(ws) -> None:
    """Fiksuoti pločiai, kad stulpeliai ne„išplauktų“ ir parašai tilptų A–E ribose."""
    ws.column_dimensions["A"].width = 6    # Eil. Nr. – siauras
    ws.column_dimensions["B"].width = 48   # Paslaugos (su wrap)
...
# ===== Vieno adreso eilės paruošimas (grupavimas pagal poreikį) =====
def prepare_rows_for_address(dfa: pd.DataFrame, service_col: str, qty_col: str, rate_col: str, group_same: bool):
    dfp = dfa.assign(
...
def write_address_block(
    ...
    - tuščią eilutę
    - lentelės antraštę
    - eilutes (Eil. Nr., Paslauga, Kiekis, Įkainis, Suma)
    - subtotal eilutę (+ grąžina jos langelio ref)
    """
    r = start_row
...
def build_workbook_act_multi(
    ...
    Toliau – blokai kiekvienam adresui, tada bendra suma ir parašų blokas A–E ribose.
    """
    ...
    if manager and col_map.get("vadybininkas"):
        dfc = dfc[dfc[col_map["vadybininkas"]].astype(str).str.strip() == manager]

    if dfc.empty:
        raise ValueError("Pagal pasirinktą sutartį/vadybininką įrašų nerasta.")
    ...
    # Meta (užsakovas/vykdytojas) – imame iš pirmos eilės
    ...
    # Turėsime skyrių map'ą rodymui (UI) — bet čia nepamirštam į lape
    dep_series = dfc[col_map["skyrius"]].astype(str).str.strip() if col_map.get("skyrius") else None

    for addr in addresses_selected:
        dfa = dfc[dfc[col_map["address"]].astype(str).str.strip() == addr].copy()
        department = str(dfa.iloc[0][col_map["skyrius"]]).strip() if dep_series is not None and not dfa.empty else ""
        # Paruošiame eilutes (grupavimas pagal poreikį)
        rows = prepare_rows_for_address(dfa, col_map["service"], col_map["qty"], col_map["rate"], group_same)
    ...
    ws[f"E{r}"] = f"=SUM({','.join(subtotal_refs)})" if subtotal_refs else 0
    ws[f"E{r}"].number_format = FMT_MONEY
    set_borders(ws, f"D{r}:E{r}", thick=True)
    r += 1
...
if col_map.get("sutartis"):
    ...
    if not sut_list:
        st.error("Kataloge nėra sutarties numerių.")
        st.stop()
    contract_no = st.selectbox("Sutarties numeris", sut_list, index=0)
    ...
    # Jei stulpelio nėra – leidžiame suvesti ranka ir tęsti be filtro
...
if col_map.get("vadybininkas"):
    with st.expander("Filtras pagal vadybininką (pasirenkama)", expanded=False):
        ...
        if mgr_list:
            ...
            st.caption("Ši sutartis neturi priskirtų vadybininkų arba laukas tuščias.")

# ===== Adresų pasirinkimas (multiselect; visi – iš tos pačios sutarties/vadybininko filtro) =====
...
if addresses_selected:
    # Rodyti skyrius pasirinktiems adresams (UI informacinis)
    if col_map.get("skyrius"):
        dep_map = (
            df[[col_map["address"], col_map["skyrius"]]]
            .dropna()
            .assign(addr=lambda d: d[col_map["address"]].astype(str).str.strip(),
                    dep=lambda d: d[col_map["skyrius"]].astype(str).str.strip())
            .drop_duplicates(subset="addr")
            .set_index("addr")["dep"]
            .to_dict()
        )
        st.markdown("**Pasirinktų adresų skyriai:**")
        for a in addresses_selected:
            st.markdown(f"- {a} — _{dep_map.get(a, 'nenurodytas')}_")
...
if btn:
    try:
        wb = build_workbook_act_multi(df_raw, col_map, contract_no or "", (None if (manager in (None, '(visi)')) else manager),
                                      addresses_selected, akto_data, group_same)
