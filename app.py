...
def create_named_styles(wb: Workbook) -> None:
    """
    Suderinama su openpyxl 3.0.x ir 3.1.x: wb.named_styles gali būti objektų arba eilučių sąrašas.
    """
    existing: set = set()
    try:
...
def write_address_block(
    ...
    for i, (service, qty, rate) in enumerate(rows, start=1):
        ...
        ws[f"D{r}"].number_format = "#,##0.0000"  # Įkainis (be PVM) – 4 skaitmenys po kablelio
